...
descriptors.
  ...
  persist   = mean # distinct problems each script was RUN on (breadth of application)
  reuse     = mean # distinct problems each script was BENEFICIAL toward (correct output submitted)
...
def main():
    ...
    hdr = (f"{'model':<26}{'sess':>5}{'solve':>7}{'effScript':>10}{'effHand':>9}"
           f"{'persist':>8}{'reuse':>7}{'recog':>7}{'scripts':>9}{'runs/prob':>10}{'tok':>9}")
    ...
    for model, rs in sorted(by_model.items()):
        ...
        line = (
            ...
            f"{_mean([r.get('persistence', r.get('mean_reuse')) for r in rs]):>8.2f}"
            f"{_mean([r.get('reusability') for r in rs]):>7.2f}"
    ...
    print("\nsolve=exact match @ sig figs; effScript=Efficiency=P(correct|used a script); effHand="
          "P(correct|by hand); persist=mean distinct problems each script was RUN on; reuse=mean "
          "distinct problems each script was BENEFICIAL toward; recog=fraction of script-used "
          "problems NOT solvable by hand (tool necessity; run recognition.py first to populate); "
          "scripts=mean written vs budget; runs/prob=mean run_script calls per problem.")
