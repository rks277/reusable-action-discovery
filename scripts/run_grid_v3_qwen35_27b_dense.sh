#!/usr/bin/env bash
# Qwen3.5-27B on toolworld v3 (grid_v3), DENSE mode, via vLLM on one H100 80GB.
# BF16 27B ~54GB fits on H100 80GB. Text-based (no tool-call flags).
# --enforce-eager avoids the Qwen3.5 hybrid-arch CUDA-graph hang.
# enable_thinking=False is injected via extra_body in raw_chat.py for qwen3* on vLLM.
...
#   && setsid nohup bash scripts/run_grid_v3_qwen35_27b_dense.sh > logs/grid_27b.out 2>&1 < /dev/null &
set -euo pipefail; trap '' HUP
REPO="$(cd "$(dirname "$0")/.." && pwd)"
PYTHON="$REPO/.venv/bin/python"; LOG="$REPO/logs/grid_27b.log"
...
setsid nohup "$REPO/.venv/bin/vllm" serve "$MODEL" --port $PORT \
    ...
    > "$REPO/logs/vllm_grid_27b.log" 2>&1 < /dev/null &
VLLM_PID=$!
...
log "Running grid_v3 DENSE sweep for $MODEL (VLLM_CONCURRENCY=64)"
cd "$REPO"
VLLM_BASE_URL="$VLLM_URL" VLLM_CONCURRENCY=64 PYTHONPATH="$REPO" \
