"""Three-panel SOLVE-RATE map for the tool-efficiency tests (Haiku / Sonnet / Opus),
one figure, Gaussian-pooled. Companion to plot_efficiency_panels.py (which shows
redundant-action waste) and a direct analogue of plot_solve_panels.py.

In these tests the machine is FORCED to be built at the start of the continuation, so
every episode is guaranteed to hold the tool. The question each cell answers is then
purely: handed the built tool, did the model finish (open ALL the doors)? Each cell is
1 if that episode solved, 0 if not (single rep/cell -> 0/1, pooled into a surface).
Green = solved with the tool in hand, red = failed to finish despite holding it.

Includes every non-error continuation (degenerate / reconstructed rows count as 0, since
they did not solve). Usage:
  PYTHONPATH=. python -m scripts.plot_efficiency_solve_panels <haiku_dir> <sonnet_dir> \
