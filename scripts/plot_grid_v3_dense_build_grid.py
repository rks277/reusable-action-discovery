"""3x3 grid of build-rate curves -- one subplot per (n, T) cell.

Each subplot plots the unconditional build rate = P(built machine) against model
size, with one line per model family (same style as plot_grid_v3_dense_lines):
x is size in B params on a log axis, Anthropic sizes are supposed estimates.
Rows are n in {9,10,11}; columns are budget T in {2,3,4}. 95% Wilson CIs.

(Companion to plot_grid_v3_dense_recog_grid, which plots R = P(built | both).)

Usage: PYTHONPATH=. python -m scripts.plot_grid_v3_dense_build_grid [dir ...]
"""
        ...
        for j, t in enumerate(TS):
            ...
            for fam, members in fams.items():
                ...
                for size, label, facts in members:
                    # build rate denominator: ALL episodes in this cell.
                    sel = [f for f in facts if f["n"] == n and f["t"] == t]
            ...
            if j == 0:
                ax.set_ylabel("P(built) -- build rate", fontsize=9)
            if i == 0 and j == 0:
                ax.legend(loc="lower left", fontsize=8)

    fig.suptitle("toolworld v3 dense -- build rate = P(built machine) per (n, T) cell "
                 "(line = family, log x; Anthropic sizes supposed; 95% Wilson CI)",
                 fontsize=14)

    out = DENSE_DIR / "fig_grid_v3_dense_build_grid.png"
