"""Haiku-only build-timing sweep: does it build the tool, and when?

Haiku counterpart of run_opus_build_sweep.py / run_sonnet_build_sweep.py -- SAME
world (n=12, T=3), SAME announced budgets, SAME reps, and stop_on_build=True, so
all three episodes.jsonl files are directly comparable (the only quantity of
interest is the per-budget proportion of runs that BUILD the machine). This adds
Haiku 4.5 as the third capability point: Haiku -> Sonnet -> Opus, the cleanest
within-family capability axis for the discovery-vs-budget question.

Like the Opus version, the hallucination/runaway-cost SAFEGUARD (no_progress_
window) is wired in. Haiku is unlikely to exhibit Opus's confabulate-the-
environment spiral (that is the big-model failure mode; see HALLUCINATION_
FINDINGS.md), but as the weakest model it is the most likely to FLOUNDER -- brute-
forcing actions up to the budget cap with no real progress. The safeguard aborts
any episode that goes NO_PROGRESS_WINDOW actions with no new byproduct type, no
new key, no new door, and no build, capping cost on episodes that were never going
to build (recorded built=False, stopped_reason="no_progress"). A genuine builder
always trips one of those before the window, so the safeguard never converts a
would-be builder into a false non-builder.

Rep r => (relabel_seed=r, drop_seed=r), shared across cells (paired worlds) and
identical to the Sonnet/Opus sweeps' worlds.

Usage: PYTHONPATH=. python -m scripts.run_haiku_build_sweep
       HAIKU_BUILD_RESUME=runs/<dir> PYTHONPATH=. python -m scripts.run_haiku_build_sweep
"""
...
# --- the sweep (matched to run_opus_build_sweep.py) --------------------
N = 12                                                       # constant doors
T = 3                                                        # constant types
...
# stay matched with the Sonnet/Opus build sweeps.
REPS_BIG = 10
...
MODELS = [("anthropic", "claude-haiku-4-5")]                 # Haiku only
...
async def main():
    load_dotenv()
    # Resume into an existing run dir if HAIKU_BUILD_RESUME points at one; this
    # lets us retune reps mid-sweep without re-spending on completed episodes.
    resume = os.environ.get("HAIKU_BUILD_RESUME")
    if resume:
        out_dir = Path(resume)
        out_dir.mkdir(parents=True, exist_ok=True)
    else:
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        out_dir = Path("runs") / f"haiku_build_sweep_T{T}_n{N}_{ts}"
        out_dir.mkdir(parents=True, exist_ok=True)
    ...
    print(f"Writing to {out_path}\n"
          f"Sweep: Haiku x budget reps {plan} (n={N}, T={T}); "
    ...
    async def one(prov, model, budget, rep):
        async with sems[prov]:
            ...
            async with lock:
                ...
                print(f"  haiku b={budget:<4} rep={rep:<2} "
...
def display_results(rows: list[dict]) -> None:
    ...
    print(f"\nHaiku build timing (n={N}, T={T}; {REPS} reps/budget, "
