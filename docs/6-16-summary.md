...
3. **Tool *discovery* isn't an existing benchmark category** — the field measures tool use / selection / creation, not discovering an un-enumerated, obfuscated affordance under a budget.
4. **Closest prior art is RL**, not LLM-agent, benchmarks (Crafter/Craftax/MineDojo/NetHack, hide-and-seek, PHYRE). Toolworld = "Crafter's discover-to-craft loop, obfuscated and run as an in-context capability/budget probe."
5. **Design proposed below:** an obfuscated, budget-bounded, text-interfaced **Crafter** as the second environment.
...
![Recognition panels](../figs/fig_recognition_panels_Nle20.png)
...
### 1.2 Playground experiment — does prior experience of *building* recover the larger models?
...
![Opus playground experiment](../figs/fig_playground_experiment_opus_n12_T6.png)
![Sonnet playground experiment](../figs/fig_playground_experiment_sonnet_n12_T6.png)
![Haiku playground experiment](../figs/fig_playground_experiment_haiku_n12_T6.png)
...
| Region analysis / plots | `scripts/analyze_nt_sweep.py`, `scripts/plot_haiku_region_heatmap.py`, `scripts/plot_region_diff.py`, `scripts/plot_build_vs_grind_region.py`, `scripts/plot_recognition_panels.py` |
| Region run data | `runs/{haiku,sonnet,opus}_region_sweep_*_Nle20_*/episodes.jsonl` (+ `points.json`, `region_*_density*.json`) |
| Region figures | build-rate `figs/{model}_region_p*/fig_*_region_build_rate_Nle20.*`; Gaussian-pooled density `figs/{model}_region_p*/fig_*_density_built_Nle20_gaussian1*.*`; recognition panels `figs/fig_recognition_panels_Nle20.*`; diffs `figs/_diffs/fig_*_minus_*_density_Nle20.*`; analytic boundary `figs/_analytic/fig_build_vs_grind_region.*` |
...
| Playground run data / figs | `runs/{opus,sonnet,haiku}_playground_expt_T6_n12_*/` (+ `design.json`), `figs/fig_playground_experiment_{opus,sonnet,haiku}_n12_T6.*`; demo pool `playgrounds/T3_n8/` |
