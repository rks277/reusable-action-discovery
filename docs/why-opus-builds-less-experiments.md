...
- **(H) Confab-suppression on the same failed seeds.** Add a grounding line ("trust only the literal OBS; never assume an action succeeded") and re-run only the seeds Opus failed. Build-rate jump → confab is causal, not incidental. Also the cheapest test of whether the whole effect is a fixable harness artifact.
...
Start with **A, B, C** on existing data (free). Prior (from the confab finding): B+C likely show Opus often *had* a buildable path and derailed into confabulation rather than failing to discover — i.e. the headline is "discovery intact, budget captured by confabulation," not "worse at discovery." That directly answers the "is it discovery or budget-allocation?" confound a reviewer will raise. Then **(H)** is the decisive follow-up: if grounding recovers Opus, the mechanism is proven *and* shown not to be incompetence.
...
**(F) oracle-recipe** and **(H) confab-suppression/grounding** on the failed seeds remain the decisive interventions; given (A)/(E), (H) reframes as a *commitment* prompt ("you hold the components — try building") rather than anti-confab grounding. Also worth: recognition rate restricted to b≤40 (deficit-only) vs b≥80 (where solve-without-build dominates).
