...
3. **Phase 3 training + eval — PARTIAL RESULT (2026-07-04); full detail in `docs/qwen-finetune-transfer-plan.md`.** SFT done (QLoRA, HF backend; bf16 OOM'd). **Held-out urn A2 (n=24): clean behavioral win** — π\*-treatment reserves (first-sight 76→54%, lateness 0.36→0.615, traps 0.62≈π*), eager-control fully eager (100%/0, traps 1.08); opposite directions rule out "any-SFT." But regret/balls ~equal (both 88% of π\*, lateness⊥regret). **Tool-transfer eval BLOCKED**: urn-heavy TEXT training (170/176 sessions) suppressed tool-calling → FT reverts to CoT in the tool framing; needs a re-train with a modality fix (design call). Follow-ups: temp=0 greedy urn readout; a_script recalib. **RESULT (2026-07-05) — TRANSFER FAILS: clean FRAMING WALL.** Option (c) anchor + a tools-block fix, pistar-only, eval on the same model via f16 GGUF→Ollama. **urn A2 (n=24): 7% first-sight, lateness 1.51, 101% of π\* balls, regret ≈0 → reserves near-optimally (stronger than the pre-anchor 54%/0.615/88% partial install). tool A2 (n=10, 2 errored): 100% first-sight, lateness 0.000 → fully eager.** Same fine-tuned model reserves ≈π\* in the urn but stays 100% eager dressed as tool-writing — the reserve policy installs but does NOT cross the framing gap. Tool eval NOISY (2/12 errored, 0.8 builds/seed, f16 GGUF artifact); lateness metric robust; clean tool rerun = follow-up. **3 process findings (fixed): (1) the real blocker was a tools-block train/eval mismatch — `build_example` rendered tool demos without `tools=` so the model never saw the eval's `<tools>` block (fixed via `_session_tools`), NOT anchor volume; (2) cuDNN SDPA backward crash on the 21k-tok session (fixed: `load_hf` disables cudnn_sdp); (3) vLLM+hermes can't parse Qwen-Coder's markdown tool calls (base fails too) → eval via GGUF→Ollama, not vLLM.** Full detail in `docs/qwen-finetune-transfer-plan.md` RESULT block. Anchor infra (`anchor_tool.jsonl` 150 build-only sessions, `train_lora.py --anchor`) as before; eager-FT still skipped (conditional follow-up).
