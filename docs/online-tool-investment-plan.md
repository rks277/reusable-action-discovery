...
**Status (2026-07-05): core result CONFIRMED + Phase 3 (fine-tune transfer) DONE — a clean FRAMING WALL.** A same-information audit added an N-disclosed arm; the headline survived it and got *stronger*. Phase 3 then SFT-installed the reserve policy into Qwen-14b: it reserves near-optimally in the urn (101% of π\* balls) but stays 100% eager in the tool framing — the competence installs but does NOT cross the framing gap (§7). Pre-audit docs in `docs/old/`. Open follow-up = a publication-grade (clean tool-call) rerun of the noisy tool eval.
