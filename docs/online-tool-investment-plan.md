...
**Status (2026-07-08): core result CONFIRMED + Phase 3 (SFT transfer) DONE (framing wall) + Phase 4 (RL) DONE, urn-side AND tool-transfer — framing wall holds for RL too.** A same-information audit added an N-disclosed arm; the headline survived it and got *stronger*. Phase 3 SFT-installed the reserve policy into Qwen-14b: reserves near-optimally in the urn (101% of π\* balls) but stays 100% eager in the tool framing — competence installs but does NOT cross the framing gap (§7). **Phase 4 (RL, `docs/rl-ppo-credit-assignment-spec.md`): RL from the untouched base, per-decision PPO+critic, no demonstrations, LEARNED to reserve — paired held-out eval 75%→32% first-sight, 87%→101% of π\* balls (matches SFT's install, but reward-discovered).** **The decisive tool-transfer eval is now DONE (2026-07-08 night): it does NOT cross the framing gap either — RL-final stays eager in the tool framing (95% first-sight, lateness 0.238) despite reserving in the urn (32% / 0.903), over a verifiably clean tool channel (0 malformed / 0 unknown calls).** So a *reward-discovered, self-installed* reserve policy transfers no better than SFT's imitated one — ruling out both "it was only imitated" and "the tool channel can't carry any policy." **Full standalone results writeup (urn + tool-transfer, transcript-level mechanism, caveats): `docs/rl-phase1-results.md`** (design/method: `docs/rl-ppo-credit-assignment-spec.md`). Pre-audit docs in `docs/old/`. **Idle-tail diagnostic DONE (2026-07-09, `rl-phase1-results.md` §4.1):** the shared base/RL empty-fence idle tail is a robust 14b generation pathology — config levers (num_ctx 16384, temp 0.2) don't fix it; a new `--empty-fence-retry` harness lever partially mitigates (breaks the runaway loop, ~halves regret) but doesn't restore sustained engagement. First-sight stayed ~91–100% across every arm, so the tail never masked transfer; it strengthens the "bigger Qwen (32b)" case for a publication-grade tool rerun.
...
**Phase 3 RESULT (2026-07-05) — TRANSFER FAILS: a clean FRAMING WALL.** Option (c) anchor + a tools-block fix, pistar-only, evaluated on the same FT model via f16 GGUF→Ollama. **The same fine-tuned model reserves near-optimally in the urn (n=24: 7% first-sight, lateness 1.51, 101% of π\* balls, regret ≈0) yet stays 100% eager in the tool framing (n=10: lateness 0.000).** So the reserve policy is *installable* (urn, cleaner than the pre-anchor partial install) but does **not transfer** across the framing gap — the coding framing suppresses an allocation competence the model demonstrably has, even after being freshly taught it. This is the strong negative the plan anticipated. Tool eval was noisy (2/12 seeds errored, 0.8 builds/seed, f16 GGUF tool-call artifact) so build-count/regret are noisy while the eager-vs-reserve conclusion is robust; a publication-grade tool rerun is a documented follow-up. Full detail + the 3 process findings (tools-block train/eval mismatch [the real blocker], cuDNN SDPA crash, vLLM+hermes can't parse Qwen-Coder tool calls → eval via GGUF→Ollama) in `docs/qwen-finetune-transfer-plan.md` RESULT block. Box released 2026-07-05.
